...
def theta_results(csv_files):
    # Check sort order

    reference_list = [str(file).split("/")[-1].split("_")[1] + '_' + str(file).split("/")[-1].split("_")[2] for file in csv_files]
...
def rho_results(csv_files):
    # Check sort order

    reference_list = [str(file).split("/")[-1].split("_")[2] + '_' + str(file).split("/")[-1].split("_")[3]  for file in csv_files]
...
if __name__ == "__main__":
    ...
    # Process the files
    theta_combined_df = theta_results(theta_files)
    rho_combined_df = rho_results(rho_files)

    # Merge results
    merged_df = pd.merge(
        theta_combined_df, rho_combined_df, 
