...
def collect_results_sweep_1(rho, theta, sample_size, depth, genome_size, seed, tract):
    ...
    recom_est_results_dir = f"/home/sid/Github/rhometa/Rho_Est_Output/rho_estimate/"

    col_names = ["rho_sim","theta_sim", "sample_size_sim", "depth_sim", "genome_size_sim",
                    "seed_sim","rho","log_likelihood_sum","genome_length","recom_tract_len","per_site_rho","subst_probability"]

    df_recom_est_results = pd.DataFrame(columns=col_names)
    ...
    for rho, theta, sample_size, depth, genome_size, seed in sweep_1_combinations:
        ...
        if os.path.isfile(f"{recom_est_results_dir}{prepended_filename}rho_estimate.csv"):
            ...
            df_recom_est_results.loc[len(df_recom_est_results)] = to_append
...
if __name__ == '__main__':
    ...
    rho_sweep_1 = [0.005, 0.01,0.02]  # unscaled r values. rho = 2 . p . N_e . r . tractlen
    theta_sweep_1 = [0.005]  # unscaled
    sample_size_sweep_1 = [20]
    depth_sweep_1 = [4]
    genome_size_sweep_1 = [50000]
    seed_sweep_1 = [1,2,3,4,5]
    ...
    reorder_cols = ["rho_sim", "scaled_rho_sim", "theta_sim", "sample_size_sim", "depth_sim", "genome_size_sim",
                    "seed_sim","rho","per_site_rho","recom_tract_len","subst_probability"]

    collected_results_sweep_1_df = collected_results_sweep_1_df.reindex(columns=reorder_cols)

    collected_results_sweep_1_df.to_csv("collected_rho_results_subsample.csv", index=None)

    collected_results_sweep_1_df = collected_results_sweep_1_df.astype('float64')

    collected_results_sweep_1_df.rename(columns={'sample_size_sim': 'genomes', 'depth_sim': 'fold_coverage', 'rho': 'rho_est'},
    ...
    ax = sns.catplot(data=collected_results_sweep_1_df, x="scaled_rho_sim", y='per_site_rho', hue="genomes",
                     col="fold_coverage", col_wrap=1, sharex=True, sharey=True, palette="Greens", kind="box",linewidth=0.2)

    # ax.set(ylim=(0, 50))
    ax.set(xlabel="Simulated \u03C1", ylabel="Estimated \u03C1 per site")
    ...
    ax.savefig("rhometa_rho_results.png", dpi=500)

    #### deviation plot

    collected_results_sweep_1_df["deviation"] = (collected_results_sweep_1_df['per_site_rho'] -
    ...
    ax2.savefig("rhometa_rho_est_deviation_results.png", dpi=500)
