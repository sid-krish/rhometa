...
def collect_results_sweep_1(rho, theta, sample_size, depth, genome_size, seed, tract):
    ...
    theta_est_results_dir = f"/home/sid/Github/rhometa/Theta_Est_Output/theta_estimate/"
    ...
    for rho, theta, sample_size, depth, genome_size, seed in sweep_1_combinations:
        ...
        if os.path.isfile(f"{theta_est_results_dir}{prepended_filename}angsd_theta_final.txt"):
            df = pd.read_csv(f"{theta_est_results_dir}{prepended_filename}angsd_theta_final.txt", header=None)
            df.columns = ["angsd_theta"]

            final_result = df.iloc[0].to_list()

            to_append = [rho, theta, sample_size, depth, genome_size, seed] + final_result
            df_theta_est_results.loc[len(df_theta_est_results)] = to_append

        else:
            print(f"{theta_est_results_dir}{prepended_filename}angsd_theta_final.txt")
            results_final = [np.nan]
...
if __name__ == '__main__':
    ...
    ax = sns.catplot(data=collected_results_sweep_1_df, x="scaled_theta_sim", y="angsd_theta", hue="genomes",
                     col="fold_coverage", col_wrap=1, sharex=True, sharey=True, palette="Blues", kind="box")

    ax.set(xlabel="Simulated \u03B8", ylabel="Estimated \u03B8 (angsd)")
    ...
    ax.savefig("rhometa_angsd_theta_est_results.png", dpi=500)

    #### deviation plot

    collected_results_sweep_1_df["deviation"] = (collected_results_sweep_1_df["angsd_theta"] -
    ...
    ax2.savefig("rhometa_angsd_theta_est_deviation_results.png", dpi=500)
